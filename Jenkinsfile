   ...
   stage('Code Quality - Sonarqube') {
      ...
      withSonarQubeEnv('Sonarqube') {
          if (env.BRANCH_NAME.startsWith('PR')) {
             ...
             sh "mvn -Dsonar.pullrequest.github.repository='nerfeboi/pibba' -Dsonar.pullrequest.key=${env.CHANGE_ID} -Dsonar.pullrequest.branch='${env.BRANCH_NAME}-sit' -Dsonar.projectKey='${FINAL_BRANCH}-sit' -Dsonar.projectName='${FINAL_BRANCH}-sit' sonar:sonar"
              ...
              // sh "mvn -Dsonar.projectKey='${FINAL_BRANCH}-uat' -Dsonar.projectName='${FINAL_BRANCH}-uat' sonar:sonar"                      
               sh "mvn -Dsonar.branch.longLivedBranches.regex='(branch|release|${FINAL_BRANCH}-sit).*' -Dsonar.branch.name='${FINAL_BRANCH}-sit' -Dsonar.projectKey='${FINAL_BRANCH}-sit' sonar:sonar"
