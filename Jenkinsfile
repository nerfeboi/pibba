   ...
   stage('Code Quality - Sonarqube') {
      ...
      withSonarQubeEnv('Sonarqube') {
          if (env.BRANCH_NAME.startsWith('PR')) {
             ...
             sh "mvn -Dsonar.branch.target=master -Dsonar.pullrequest.key=${env.CHANGE_ID} -Dsonar.pullrequest.branch='${env.BRANCH_NAME}-sit' -Dsonar.projectKey='${FINAL_BRANCH}-sit' -Dsonar.projectName='${FINAL_BRANCH}-sit' sonar:sonar"
