               ...
               withSonarQubeEnv('Sonarqube') {
                  script{
                       if (env.BRANCH_NAME.startsWith('PR')) {
                         ...
                         sh "mvn -Dsonar.verbose=true -Dsonar.pullrequest.github.repository='nerfeboi/pibba' -Dsonar.pullrequest.key=${env.CHANGE_ID} -Dsonar.pullrequest.branch='${env.BRANCH_NAME}' -Dsonar.projectKey='${FINAL_BRANCH}-uat' -Dsonar.projectName='${FINAL_BRANCH}-uat' sonar:sonar"
